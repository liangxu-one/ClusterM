...
def greedy_decode(model_list, img, caption_index, config):
    ...
    for model in model_list:
        img_embed = model(img)
        image_embed_list.append(img_embed)
    for i in range(config.generation_length):
        ...
        for j in range(len(model_list)):
            pred = model_list[j](img, caption_index, caption_mask, img_embed = image_embed_list[j])
            pred_list.append(pred)
        pred = torch.stack(pred_list).softmax(dim = -1).mean(dim = 0)
        next = pred[:, -1, :].log().max(-1)
...
def beam_decode(model_list, img, caption_index, config):
    bs = img.size(0)
    ...
    out_list = []
    for j in range(len(model_list)):
        out = model_list[j](img, caption_index, caption_mask, img_embed = image_embed_list[j])
        out_list.append(out)
    out = torch.stack(out_list).softmax(dim = -1).mean(dim = 0)
    next = out[:, -1, :].log()
